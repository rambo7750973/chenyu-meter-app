...
interface MeterDao {
    ...
    @Query("SELECT SUM(dailyEnergy) FROM meter_readings WHERE meterId = :meterId AND timestamp BETWEEN :startTime AND :endTime")
    suspend fun getTotalEnergyByTimeRange(meterId: String, startTime: Long, endTime: Long): Float?
